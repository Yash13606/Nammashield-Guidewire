...
export default function AdminPage() {
  ...
  useEffect(() => {
    ...
    const poll = async () => {
      try {
        const since = payoutCursorRef.current;
        const res = await fetch(`/api/admin/payout-events?since=${encodeURIComponent(since)}&limit=20`);
        if (!res.ok) return;
        ...
        for (const ev of events) {
          if (payoutSeenRef.current.has(ev.id)) continue;
          payoutSeenRef.current.add(ev.id);

          const who = ev.worker_phone ? ` to ${ev.worker_phone}` : "";
          if (ev.status === "completed") {
            toast({
              title: `Payout sent${who}: ₹${Number(ev.amount).toLocaleString("en-IN")}`,
              description: `Channel: ${(ev.channel ?? "UPI_SIM").replace(/_/g, " ")} · ${ev.processed_at ? new Date(ev.processed_at).toLocaleString() : "processed"}`,
            });
          } else if (ev.status === "failed") {
            toast({
              title: `Payout failed${who}`,
              description: ev.failure_message ?? "Mock gateway returned failure",
              variant: "destructive",
            });
          }
        }
        ...
        payoutCursorRef.current = last.processed_at ?? last.requested_at;
      } catch {
        // Keep admin polling silent on transient network errors.
      }
    };
  ...
  return (
    <div className="max-w-7xl mx-auto pb-10">
      ...
      {/* ── Fraud Audit Trail ── */}
      <motion.div className="bg-white rounded-2xl p-6 mb-6" style={{ boxShadow: cardShadow }} {...fadeUp(0.258)}>
        <h3 className="text-sm font-semibold mb-1" style={{ fontFamily: "var(--font-body)" }}>
          Fraud Audit Trail
        </h3>
        <p className="text-xs mb-4" style={{ color: "var(--muted)", fontFamily: "var(--font-mono)" }}>
          Recent anomaly reason codes generated by the fraud engine
        </p>
        {(summary?.flaggedFraudCases?.length ?? 0) === 0 ? (
          <p className="text-sm" style={{ color: "var(--muted)" }}>No recent fraud logs.</p>
        ) : (
          <div className="overflow-x-auto">
            <table className="w-full text-sm">
              <thead>
                <tr style={{ color: "var(--muted)", fontFamily: "var(--font-mono)", fontSize: "0.6875rem" }}>
                  <th className="text-left pb-2 uppercase tracking-wider">Time</th>
                  <th className="text-left pb-2 uppercase tracking-wider">Claim</th>
                  <th className="text-left pb-2 uppercase tracking-wider">Reason</th>
                  <th className="text-left pb-2 uppercase tracking-wider">Severity</th>
                </tr>
              </thead>
              <tbody>
                {(summary?.flaggedFraudCases ?? []).slice(0, 10).map((row) => (
                  <tr key={`${row.claim_id}-${row.created_at}-${row.reason_code}`} className="border-t border-[var(--border)]">
                    <td className="py-2.5 text-xs" style={{ color: "var(--muted)" }}>
                      {new Date(row.created_at).toLocaleString()}
                    </td>
                    <td className="py-2.5" style={{ fontFamily: "var(--font-mono)" }}>
                    ...
                    <td className="py-2.5">
                      <span className="inline-flex px-2 py-0.5 rounded-full text-[10px] font-semibold"
                        style={{
                          background: row.severity === "high" ? "#FEE2E2" : "#E0F2FE",
                          color: row.severity === "high" ? "#DC2626" : "#0369A1",
                        }}>
                        {row.severity}
                      </span>
                    </td>
                  ...
                  {[
                    {
                      ...
                      value: String(
                        (simResult as { affected_workers?: unknown }).affected_workers ??
                          (simResult as { workers_found?: unknown }).workers_found ??
                          "—"
                      ),
                    },
                    {
                      ...
                      value: String(
                        (simResult as { claims_created?: unknown }).claims_created ??
                          (Number((simResult as { auto_approved?: unknown }).auto_approved ?? 0) +
                            Number((simResult as { watchlist?: unknown }).watchlist ?? 0) +
                            Number((simResult as { flagged?: unknown }).flagged ?? 0) +
                            Number((simResult as { rejected?: unknown }).rejected ?? 0))
                      ),
                    },
                    {
                      label: "Total Payout",
                      value: (simResult as { total_payout?: unknown }).total_payout
                        ? `₹${Number((simResult as { total_payout?: unknown }).total_payout).toLocaleString("en-IN")}`
                        : "—",
                    },
                  ].map(({ label, value }) => (
                    <div key={label}>
